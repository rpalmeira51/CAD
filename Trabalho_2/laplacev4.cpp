...
void print_array(Real* a, std::ostream& o = std::cout)
{
  o << "{";
  for (std::size_t i = 500*250; i < (500)*251; ++i)
  {
    o << a[i] << ", ";
  }
  o  << "}\n";
...
struct LaplaceSolver{
	...
	~LaplaceSolver();
	void initialize();
	Real timeStep(const Real dt=0.0, int step_size=0);
...
Real LaplaceSolver :: timeStep(const Real dt, int step_size)
{
...
Real LaplaceSolver :: solve(const int n_iter, const Real eps)
{
	int count = 1;
	int step_size = 5;
	int max_iter = n_iter/step_size;
	Real err = timeStep(0.0,step_size);
	while (err > eps) {
		if (n_iter && (count >= max_iter)) {
			return err;
		}
		err = timeStep(0.0,step_size);
		++count;
	}
	return Real(count);
}
