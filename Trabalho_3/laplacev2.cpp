...
Real LaplaceSolver :: timeStep(const Real dt)
{
	...
	int ny = g->ny;
	Real **u = g->u;
	#pragma omp parallel for shared(u,nx,ny,dx2,dy2) private(tmp) reduction( + : err)
	for (int i=1; i<nx-1; ++i) {
		for (int j=1+(i%2); j<ny-1; j += 2) {
			tmp = u[i][j];
...
int main(int argc, char * argv[])
{
	int nx, n_iter;
	Real eps;
	Real t_start, t_end,t_start_omp,t_end_omp;
